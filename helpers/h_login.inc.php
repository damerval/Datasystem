  ...
  if (!function_exists("jys_lookupUser")) {
		function jys_lookupUser($ulogin) {
			...
			$sql = "SELECT DISTINCT SQL_LOGIN, USER_FNAME, USER_LNAME, USER_NUMBER, sys.server_principals.principal_id,
							CASE WHEN UPPER(PC.ContactType) IN ('PC', 'PD', 'SS') OR (JYS_USERS.USER_NUMBER=167) THEN 'SS' ELSE '00' END AS Supervisor, USER_LOGIN,
							...
							LEFT JOIN dbo.ProjectContact PC ON PC.Employee=USER_NUMBER and UPPER(PC.ContactType) IN ('SS', 'PC', 'PD', 'IT') AND PC.Active<>0
							LEFT JOIN dbo.EMPLOYEE ON USER_NUMBER=dbo.EMPLOYEE.EMPLOYEE_ID
	...
	if (!function_exists("jys_authenticate")) {
		function jys_authenticate($uname, $pword) {
			require_once($_SERVER["DOCUMENT_ROOT"] . "/data/host.php");
